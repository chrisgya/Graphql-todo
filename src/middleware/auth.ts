...
export const isAuthenticated = (context) => {
  const promise: Promise<Response> = new Promise((resolve, reject) => {
    ...
    if (!token) {
      ...
      jwt.verify(token, secret, (err, decoded) => {
        if (err) {
          reject(new Response(500, "Unable to authenticate user", { success: false }));
        } else {
          User.findById(decoded.id).select("password").then((user) => {
            if (!user) {
              reject(new Response(400, "Sorry No user found", { success: false }));
            } else {
  ...
  return completeRequest(promise);
};
